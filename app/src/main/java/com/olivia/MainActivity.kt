...
//IMAGE BUTTON
        ...
        Button(
            onClick = {inp.startActivity(Intent(inp, ExeActivity::class.java))},
            shape = RoundedCornerShape(15.dp),
            colors = ButtonDefaults.outlinedButtonColors(Color.Blue),
            border = BorderStroke(1.5.dp, Color.Cyan)
        ){
            Text("Exe", color = Color.Red)
        }


        val Web = LocalContext.current
        Button(
            onClick = {Web.startActivity(Intent(inp, WebActivity2::class.java))},
            shape = RoundedCornerShape(15.dp),
            colors = ButtonDefaults.outlinedButtonColors(Color.Blue),
            border = BorderStroke(1.5.dp, Color.Cyan)
        ){
            Text("Web", color = Color.Red)
        }

        val New = LocalContext.current
        Button(
            onClick = {New.startActivity(Intent(inp, NewActivity2::class.java))},
            shape = RoundedCornerShape(15.dp),
            colors = ButtonDefaults.outlinedButtonColors(Color.Black),
            border = BorderStroke(1.5.dp, Color.Blue)
        ){
            Text("New", color = Color.Red)
        }


        val abt = AnnotatedString("Already have an Account?Log in...", spanStyle = SpanStyle(Color.LightGray))
        val ab = LocalContext.current
        ClickableText(text = abt , onClick = {ab.startActivity(Intent(ab, AboutActivity::class.java))})
